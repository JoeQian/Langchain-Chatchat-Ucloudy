...
#     xf_url = st.text_input("Xinference endpoint", "http://192.168.170.13:9997")
#     st.divider()
#     st.markdown(
#         "### 使用方法\n\n"
...
# 获取缓存目录
def get_cache_dir(model_type: str, model_name: str, model_format: str = "", model_size: str = ""):
    if model_type == "LLM":
        dir_name = f"{model_name}-{model_format}-{model_size}b"
    else:
        dir_name = f"{model_name}"
...
# 获取元数据路径
def get_meta_path(model_type: str, cache_dir: str, model_format: str, model_hub: str = "huggingface", model_quant: str = "none"):
    if model_type == "LLM":
        return xf_llm.llm_family._get_meta_path(
            cache_dir=cache_dir,
            model_format=model_format,
...
# 异步获取当前运行的模型
def list_running_models_async():
    future = executor.submit(client.list_models)
    models = future.result()  # 等待结果
    columns = [
        "UID", "type", "name", "ability", "size", "quant", "max_tokens",
    ]
    data = []
    for k, v in models.items():
        item = dict(
...
# 异步获取模型注册信息
def get_model_registrations_async():
    future = executor.submit(client.list_model_registrations, model_types)
    registrations = future.result()
    data = {}
    for model_type in model_types:
        data[model_type] = {}
        for m in registrations[model_type]:
            data[model_type][m["model_name"]] = {"is_builtin": m["is_builtin"]}
            reg = client.get_model_registration(model_type, m["model_name"])
            data[model_type][m["model_name"]]["reg"] = reg
    return data
...
# 使用线程池获取客户端
client = get_client("http://192.168.170.13:9997")

st.subheader("当前运行的模型：")
st.dataframe(list_running_models_async())  # 异步列出运行中的模型

st.subheader("配置模型路径：")
regs = get_model_registrations_async()  # 异步获取模型注册信息
cols = st.columns([3, 4, 3, 2, 2])
